 ...
 * miente. `needsAction` es lo que decide el orden de la lista y el énfasis
 * visual, no una preferencia estética.
 */
 ...
 * ordenado por semana. Un orden por fecha de creación enterraría justo lo que
 * hay que atender.
 */
...
export function sortByRequiredAction(deliverables: readonly Deliverable[]): readonly Deliverable[] {
  return [...deliverables].sort((left, right) => {
    const leftSubmission = currentSubmission(left);
    const rightSubmission = currentSubmission(right);

    // Un agregado sin revisión vigente no puede reclamar prioridad.
    const leftPriority =
      leftSubmission === null ? Number.MAX_SAFE_INTEGER : priorityByStatus[leftSubmission.status];
    const rightPriority =
      rightSubmission === null ? Number.MAX_SAFE_INTEGER : priorityByStatus[rightSubmission.status];
...
export function canSubmitRevision(submission: Submission): SubmitBlock {
  ...
  if (scanning > 0) {
    const plural = scanning === 1 ? 'archivo' : 'archivos';
    return { canSubmit: false, reason: `Esperando el análisis de ${scanning} ${plural}.` };
  }

  const blocked = submission.files.filter((file) => file.scanStatus !== 'CLEAN').length;
  if (blocked > 0) {
    const plural = blocked === 1 ? 'archivo' : 'archivos';
    return { canSubmit: false, reason: `Quita ${blocked} ${plural} que no se pudo procesar.` };
  }
